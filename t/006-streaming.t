...
my ($duid, $err) = Net::DHCPv6->decode_duid_with_error(pack('n n N a*', 1, 1, 123456, $mac));
...
is($duid->link_layer_type, 1, 'full DUID-LLT hwtype');
...
($duid, $err) = Net::DHCPv6->decode_duid_with_error(pack('n n', 1, 1));
...
is($duid->link_layer_type, 1, 'partial DUID-LLT has hwtype');
...
$duid = Net::DHCPv6->decode_duid_or_null(pack('n n N a*', 1, 1, 123456, $mac));
ok(defined $duid, 'decode_duid_or_null full returns duid');

# decode_duid_or_null — partial (returns partial, not undef)
$duid = Net::DHCPv6->decode_duid_or_null(pack('n n', 1, 1));
...
$duid = Net::DHCPv6->decode_duid_or_croak(pack('n n N a*', 1, 1, 123456, $mac));
ok(defined $duid, 'decode_duid_or_croak full returns duid');

# decode_duid_or_croak — partial (croaks)
ok(dies { Net::DHCPv6->decode_duid_or_croak(pack('n n', 1, 1)) },
    'decode_duid_or_croak partial croaks');

# decode_duid_or_croak — empty (croaks)
ok(dies { Net::DHCPv6->decode_duid_or_croak('') },
    'decode_duid_or_croak empty croaks');

# ─── Options streaming helpers ─────────────────────────────────────

# Build a known good options byte string
my $cid  = Net::DHCPv6::Option::ClientId->new(duid =>
    Net::DHCPv6::DUID->new_llt(1, 123456, $mac));
my $sid  = Net::DHCPv6::Option::ServerId->new(duid =>
    Net::DHCPv6::DUID->new_llt(1, 123456, $mac));
...
ok(dies { Net::DHCPv6->decode_options_or_croak($truncated) },
    'decode_options_or_croak truncated croaks');
