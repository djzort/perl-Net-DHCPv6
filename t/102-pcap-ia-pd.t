...
my @data = (
    ['solicit',  pack("H*","01e1e0930001000a0003000100010203040500060004001700180008000200000019000c0203040500000e1000001518")],
    ['advertise',pack("H*","02e1e093001900290203040500000e1000001518001a00190000119400001c20382a0000010001010000000000000000000001000a000300010001020304050002000e0001000118464999001122334455")],
    ['request',  pack("H*","0312b08a0001000a000300010001020304050002000e00010001184649990011223344550006000400170018000800020000001900290203040500000e1000001518001a001900001c2000001d4c382a000001000101000000000000000000")],
    ['reply',    pack("H*","0712b08a001900290203040500000e1000001518001a00190000119400001c20382a0000010001010000000000000000000001000a000300010001020304050002000e0001000118464999001122334455")],
);
...
subtest 'solicit options' => sub {
    ...
    is(unpack('H*', $cid->duid->identifier), '000102030405',
        'ClientId MAC');

    my $oro = $ol->get_option(6);
    ok($oro, 'ORO present');
    is($oro->requested_options, [23, 24],
        'ORO requests DNS + Domain');
...
subtest 'advertise options' => sub {
    ...
    is($pfx->address, pack('H*', '2a000001000101000000000000000000'),
        'IAPREFIX address');
    ...
    is(unpack('H*', $sid->duid->identifier), '001122334455',
        'ServerId MAC');
};
